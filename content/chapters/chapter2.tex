...
Una volta che Vulkan è inizializzata e lo sheder è pronto per essere compilato, quello che serve è allocare le risorse per l'implementazione del programma. Prima di addentrasi nei dettagli implementativi serve capire come Vulkan alloca e gestisce la memoria.

In Vulkan, considereremo due tipi di memoria: \textit{host memory} e \textit{device memory}. La principale differenza tra le due è che la memoria host è accessibile e utilizzata dalla CPU, mentre la memoria device è accessibile solo dal dispositivo in cui viene eseguito lo shader. 

!! 

La memoria del dispositivo può avere alcune proprietà e tipi. Ad esempio, un tipo di memoria può essere \verb|HOST_VISIBLE|, il che specifica che la memoria può essere mappata per l'accesso da parte dell'host oppure \verb|DEVICE_LOCAL| che indica che la memoria è, appunto, accessibile sono al device. L'applicazione è responsabile dell'allocazione della memoria con il tipo appropriato in base alle proprie esigenze. Vulkan offre questa flessibilità per migliorare le prestazioni e l'utilizzo della memoria.

Per l'implementazione in \ref{lis:vulkan_shader} servono tre vettori, quindi tre allocazioni di memoria con il tipo di memoria \verb|HOST_VISIBLE|. I primi due vettori sono gli input, il parametro \verb|readonly| nello shader permette varie ottimizzazioni di compilazione. Il terzo vettore è il risultato della computazione, marcato \verb|writeonly| perché viene soltanto scritto nello shader. 

\vspace{5mm}
\lstinputlisting[language=Rust, caption=??, label=lis:vulkan_execution]{code/vulkan_execution.rs} 
\vspace{5mm}

Il marcare tutti i tre vettori come \verb|HOST_VISIBLE| è giustificato dal fatto che i primi due sono inizializzati dalla CPU in memoria host, mentre il terzo viene letto dalla CPU per conoscere il risultato dell'operazione. In caso di computazioni intermedie, in cui ad esempio non serve alla CPU conoscere il risultato di un'operazione ma soltanto passarlo a un altro shader per la computazione successiva, allora avrebbe senso marcare tale area di memoria come \verb|DEVICE_LOCAL| permettendo la gestione unicamente alla GPU ed evitando eventuali copie in memoria host.
