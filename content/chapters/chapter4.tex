...
Per quanto riguarda le operazioni sui dati in \ref{lis:generic_rust} è mostrato come usando il \textit{composition pattern} in Rust, è possibile implementare il tratto \verb|Add| per la somma dei vettori e implementare la \gls{FMA} per la moltiplicazione delle matrici. Il resto della logica rimane invariata rispetto a \gls{CUDA}, con una funzione \verb|benchmark::<T>(..., Operation::...(Type::T))| che specializza il tipo di dato ed esegue tutte le operazioni necessarie.
